...
From the table \ref{tab:persona_metrics}

To reduce varyation and to be able to better compare between the two chains, personas present on both chains were generated with the same distribution of first seen dates. In the same idea, similar statistical distributions were used, albeit not identical. Complex modeling based on token price evolution, market volatility and differentiated behavior was used to model each persona. A notable difference is the Ethereum Occasional User that was chosen to have appeared only after the transition to PoS, we expect fairly low emissions across all methodologies in his case even though he interacts a lot with the chain. This should contrast with the Occasional User on Bitcoin.
To note, institutional investor to showcase regular interactions and very high secured asets.
...
As with the new \ac{CCRI} hybrid allocation approach, by distributing network footprints to end-users, protocols, and DAOs, this framework enables entities to understand, report, and take action based on their nuanced responsibility in the overall emissions.
